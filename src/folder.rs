...
fn restart() {
    if Confirmation::with_theme(get_theme())
        .with_text("Do you want to search for another folder?")
        .interact()
...
fn search() -> Vec<String> {
    ...
    let folder_name = Input::<String>::with_theme(get_theme())
        ...
        Ok(mounts) => {
            for mount in mounts.iter() {
                ...
                for (index, entry) in WalkDir::new(&mount.fs_mounted_on)
                    ...
                    let directory = path.replace(&file, "");

                    if directory.contains(&folder_name) &&
